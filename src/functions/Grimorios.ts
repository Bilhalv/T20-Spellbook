import { doc, getDoc, updateDoc, arrayUnion, setDoc, onSnapshot, collection, getFirestore, getDocs, arrayRemove } from "firebase/firestore";
...
export async function addToGrimorio(x: magiaTipo, personagem: string) {
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (data.email === firebase.auth().currentUser?.email && data.personagem === personagem) {
                personagemID = doc.id;
            }
        });
...
export async function getPersonagens(setPersonagens: (personagens: string[]) => void) {
...
export async function removeGrimorio(x: string, grimorio: grimorioTipo | undefined) {
    ...
    try {
        ...
        querySnapshot.forEach((document) => {
            ...
            if (data.email === firebase.auth().currentUser?.email && data.personagem === grimorio?.personagem) {
                const docRef = doc(refGrimorio, document.id);
                updateDoc(docRef, {
                    magias: arrayRemove((grimorio?.magias.find((y) => y.nome === x) as magiaTipo))
                });
                console.log("Document written with ID: ", docRef.id);
            }
        });
...
export async function getGrimorio(setGrimorio: (grimorio: grimorioTipo | undefined) => void, personagem: string | undefined) {
    const querySnapshot = await getDocs(refGrimorio);
    querySnapshot.forEach((doc) => {
        const data = doc.data() as grimorioTipo;
        if (data.email === firebase.auth().currentUser?.email && data.personagem === personagem) {
